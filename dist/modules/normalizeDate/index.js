...
function validDay(day = '', month) {
    ...
    if (!(sanitizedDay === null || sanitizedDay === void 0 ? void 0 : sanitizedDay.length))
        return '';
    const dayNumber = Number(sanitizedDay);
    if (!month) {
        if (dayNumber > 31)
            throw errors.invalidDay(day);
        return day.padStart(2, '0');
    }
    const sanitizedMonth = onlyNumbers(month);
    const monthNumber = Number(sanitizedMonth);
    if (monthNumber > 12)
        throw errors.invalidMonth(month);
...
function normalizeYear(year) {
    ...
    if (!(sanitizedYear === null || sanitizedYear === void 0 ? void 0 : sanitizedYear.length))
        return '';
    if (sanitizedYear.length == 3 || sanitizedYear.length > 4)
        throw errors.invalidYear(year);
    ...
    if (sanitizedYear.length === 2 && sanitizedYear > lastTwoDigits)
        return `19${sanitizedYear.padStart(2, '0')}`;
...
function normalizeDayOrMonth(dayOrMonth) {
    ...
    if (!(sanitizedDayOrMonth === null || sanitizedDayOrMonth === void 0 ? void 0 : sanitizedDayOrMonth.length))
        throw errors.invalidDate(dayOrMonth);
    if ((sanitizedDayOrMonth === null || sanitizedDayOrMonth === void 0 ? void 0 : sanitizedDayOrMonth.length) > 2)
        throw errors.invalidDate(dayOrMonth);
