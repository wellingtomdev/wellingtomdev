...
function normalizeNumber(number, numberValidation = true) {
    ...
    if (!numberValidation)
        return number;
    if (getFirstsNumbers(number, 2) === '55') {
        number = number.slice(2);
    }
    if (getFirstsNumbers(number, 2) === '15' && number.length > 11) {
        number = number.slice(2);
    }
    else if (getFirstsNumbers(number, 3) === '015' && number.length > 12) {
        number = number.slice(3);
    }
    if (getFirstsNumbers(number, 1) === '0') {
